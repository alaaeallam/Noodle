...
try {
  if (process.env.NODE_ENV !== 'development') {
    // Load service account only outside dev to avoid crashes if the file is missing
    ...
    console.warn('[FCM] Disabled in DEV (NODE_ENV=development).')
  }
} catch (e) {
  console.warn('[FCM] Firebase init failed, disabling in this environment:', e.message)
...
const sendNotification = async orderId => {
  ...
  if (!firebaseEnabled) return true // silently skip notifications in dev
  if (owner.pushToken) {
    var message = {
      data: {
        orderid: orderId
...
exports.firebaseEnabled = firebaseEnabled
