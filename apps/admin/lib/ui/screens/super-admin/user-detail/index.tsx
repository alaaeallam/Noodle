...
const UserDetailScreen: React.FC<UserDetailScreenProps> = ({ userId }) => {
    ...
    // Fetch user data
    const { data: userData, loading: userLoading, error: userError } = useQuery<{ user: IUserResponse }>(GET_USER_BY_ID, {
        variables: { userId },
    ...
    if (!userId) {
        return (
            <div className="flex justify-center items-center h-screen p-4">
                <Card title="Invalid User ID" className="shadow-md border border-orange-300 bg-orange-50 text-orange-800">
                    <p className="mb-3">No user ID was provided. Please ensure you are accessing this page with a valid user ID.</p>
                    <div className="mt-4 flex justify-end">
                        <Button label="Go Back" icon="pi pi-arrow-left" className="p-button-secondary" onClick={() => router.back()} />
                    </div>
                </Card>
            </div>
        );
    ...
    if (userError || ordersError) {
        const errorMessage = userError?.message || ordersError?.message || "An unknown error occurred.";
        return (
            <div className="flex justify-center items-center h-screen p-4">
                <Card title="Error Loading User Details" className="shadow-md border border-red-300 bg-red-50 text-red-800">
                    <p className="mb-3">We encountered an issue while trying to fetch the user&apos;s data.</p>
                    <p className="font-semibold">Details: {errorMessage}</p>
                    <p className="mt-3 text-sm">Please check your network connection or contact support if the issue persists.</p>
                    <div className="mt-4 flex justify-end">
                        <Button label="Go Back" icon="pi pi-arrow-left" className="p-button-secondary" onClick={() => router.back()} />
                    </div>
                </Card>
    ...
    if (!user) {
        return (
            <div className="flex justify-center items-center h-screen p-4">
                <Card title="User Not Found" className="shadow-md border border-orange-300 bg-orange-50 text-orange-800">
                ...
                <div className="card mt-6 mb-20 border shadow-sm !rounded-xl">
                    ...
                    >
                        {tabs.map((tab, index) => (
                            <TabPanel
                                key={index}
                                headerTemplate={(options) => tabHeaderTemplate(options, tab, index)}
                            >
                                {tab.content}
                            </TabPanel>
                        ))}
