...
export default function FoodsMain() {
  ...
  } = useLazyQueryQL<ISubCategorySingleResponse, { id: string }>(
    GET_SUBCATEGORY,
    {
      fetchPolicy: 'network-only',
      debounceMs: 0,
  ...
  useEffect(() => {
    if (subcategoryError) {
      console.log({ err: subcategoryError });
    }
  }, [subcategoryError]);
  ...
  const menuItems: IActionMenuItem<IFoodNew>[] = [
    {
      ...
      command: async (data?: IFoodNew) => {
        ...
        await fetchSubcategory({ id: sub_ctg_id || '' });
        ...
        console.log('[fetchSubcategory] about to run with', { sub_ctg_id });
        const res = await fetchSubcategory({ id: sub_ctg_id || '' });
        console.log('[fetchSubcategory] returned', { res });
