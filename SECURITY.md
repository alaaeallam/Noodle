...
- **2026-08-24** — Fixed #14: user confirmed demo mode isn't used anymore. Wrote and ran `apps/api/scripts/disable-demo-mode.js` to turn off `enableRiderDemo`/`enableRestaurantDemo` in production (no admin panel UI exists to do this - the mutation that changes these flags is never actually called from the panel). Also hardened `lastOrderCreds` itself to always return `null` for the password fields regardless of the flag, so this can't leak a real credential again even if demo mode is re-enabled by someone later without reading this. Remaining open: #11 (needs manual Google Cloud Console check, not a code fix) - everything else code-fixable is done.
