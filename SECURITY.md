...
| 15 | **High** | app, store | `production`/`staging` build channels pointed `GRAPHQL_URL`/`WS_GRAPHQL_URL` at the original template vendor's dead backend domain instead of the real one — found while investigating #11 | **Fixed** (2026-08-24) |
| 16 | **High** | admin | `next@14.2.35` had ~21 unpatched CVEs (7 high — DoS, SSRF, cache poisoning, middleware/i18n bypass), none fixed anywhere in the 14.x line | **Fixed** (2026-09-04) — major upgrade to Next 15.5.25 + React 19 |
| 17 | **Medium** | app, store, rider, admin, api | Routine dependency-CVE sweep across all 5 apps (the full `npm audit` pass this doc's Scope note had flagged as not yet done) — axios, mongoose, jsonwebtoken, nodemailer, firebase-admin, ajv, lodash, `@babel/core`, postcss, and others | **Fixed** (2026-08-17 → 2026-09-04, rolling) |
| 18 | **Info** | api, app, store, rider, admin | `subscriptions-transport-ws` (unmaintained since ~2021) fully replaced with `graphql-ws` across the backend and all 4 client apps; `apollo-server-express` v3 (EOL) replaced with `@apollo/server` v5 | **Fixed** (2026-09-04) |
...
### 15. [High] Customer app and store app pointed at the vendor's dead backend on production/staging
**Where:** `apps/app/environment.js`, `apps/store/environment.ts`

Found while investigating #11 (checking why the customer app's Google Maps keys didn't match what the Google Cloud project showed — a tangent that turned up something bigger). Both apps' `useEnvVars`/`getEnvVars` functions branch on `Updates.channel`: when it's `"production"` or `"staging"` (which is exactly what `eas build --profile production`/`staging` sets, per each app's `eas.json`), `GRAPHQL_URL`/`WS_GRAPHQL_URL` pointed to `https://aws-server.enatega.com/graphql` — a domain the user confirmed doesn't belong to them, almost certainly a leftover from the original Enatega template. Any other channel (including plain local dev) correctly used `https://foodapp-api.alaaeallam.com/graphql`.

Checked whether this is a live "sends real user data to a third party" risk or something else: `aws-server.enatega.com` still resolves via DNS to a live Railway-hosted backend (`enatega-multivendor-api-production-*.up.railway.app`), but a `curl` to it fails TLS certificate validation — the certificate presented is for `*.up.railway.app`, not `aws-server.enatega.com`, so the hostnames don't match. A real device's networking stack enforces the same hostname check curl does, so the practical impact isn't a silent data leak — it's that **any build running on the production/staging channel would fail every GraphQL request outright** (no login, nothing works). `apps/rider` already had this same bug fixed previously (stale URL visible commented-out in `environment.ts`) - `apps/app` and `apps/store` still had it live in code, just never triggered because the user confirmed they aren't currently using TestFlight/App Store distribution for either.

**Fix applied 2026-08-24:** both apps' production/staging branches now point to `foodapp-api.alaaeallam.com`, matching the dev branch (and matching what rider already had). JS-only change, ships via OTA with no native rebuild needed.

### 16. [High] admin's Next.js had ~21 unpatched CVEs, no fix in the 14.x line
**Where:** `apps/admin/package.json` — `next@14.2.35`

A routine dependency-CVE audit of `apps/admin`, `apps/store`, and `apps/rider` (see #17) turned up something bigger for admin specifically: 14.2.35 was already the *newest* 14.x release available, but ~21 CVEs against `next` remained unpatched against it — 7 high severity (DoS via Server Components/Server Actions, SSRF via rewrites and WebSocket upgrades, a middleware/i18n bypass in the Pages Router) plus assorted moderate/low (cache poisoning, XSS in App Router CSP-nonce handling, unbounded image-cache growth). Checked each CVE's `patched_versions` range individually — none of them had ever been backported into the 14.x line; Next.js's own advisories only fix these starting at various points in 15.5.x. So this wasn't a routine patch-bump situation: closing it required a major-version upgrade.

**Fix applied 2026-09-04:** `next` 14.2.35 → **15.5.25** (latest stable 15.x — satisfies every one of the CVEs' patched thresholds without jumping to the still-newer, less-battle-tested 16.x line), which forced `react`/`react-dom` 18.3.1 → 19.2.8 (Next 15's minimum) and `next-intl` 3.26.5 → 4.14.2 (itself closing 2 more CVEs, see #17). Audited every dynamic route and server-side API surface for the version's breaking changes (`cookies()`/`headers()`/`params`/`searchParams` are now async) — found and fixed two real call sites (`lib/utils/methods/locale.ts` now awaits `cookies()`; the client-component `general/users/user-detail/[id]/page.tsx` unwraps `params` via React's `use()` hook). Two npm `overrides` were needed to work around genuine lockfile bugs the upgrade surfaced: `postcss` (Next 15 still bundles its own older internal copy separately from the project's own patched devDependency) and `@swc/core` (next-intl 4.x's new hard dependency had a nested per-platform-optional-binary lockfile entry with no version field, breaking `npm ci` — reproduced across two npm versions before being traced to this).

Verified via: clean `npm install`/`npm ci` passing in isolated copies with zero React-19 peer-dependency errors from any of admin's UI libraries; a full `next build` succeeding twice (58/58 routes, no compile errors); booting the *exact* standalone-output assembly `deploy-admin.yml` produces and curling every route to 200; the real VPS deploy's own `npm ci`+`npm run build` succeeding (the actual test of the lockfile fix, on production's node/npm version, not just local); and — once account access was restored (see #19) — a full authenticated browser walkthrough of the pages most exposed to the upgrade's breaking changes (the user-detail page with its changed `params` handling, the language switcher including a full RTL flip and cookie-persistence-across-navigation check, the live-subscription Dispatch page, and a PrimeReact-heavy settings form), all clean with zero console errors.

### 17. [Medium] Dependency CVEs across app, store, rider, admin, api
**Where:** all 5 apps' `package.json`/lockfiles, over several passes from 2026-08-17 through 2026-09-04

This doc's Scope note previously flagged "a full `npm audit`/dependency CVE sweep across all 5 apps" as not yet done — this closes that gap. Ran in stages: `apps/app` first (axios SSRF/credential-leak CVEs, `mongoose`, `jsonwebtoken`, `nodemailer`, `firebase-admin`, plus dead weight removed — `eslint-plugin-standard`, `metro-react-native-babel-preset`, and the unused `react-native-vector-icons` package after confirming zero import sites), then `apps/api` (same core CVE set, plus the `apollo-server-express` v3 → `@apollo/server` v5 migration — v3 was EOL and no longer receiving security patches, see #18), then `apps/store`/`apps/rider`/`apps/admin` together.

For the second batch, cross-referenced pnpm's workspace-wide `audit` output against each app's own *actually-resolved* dependency version (not just package-name matching, which produces false positives in a shared workspace lockfile where the same package can resolve to different versions per app) to find genuinely-applicable, low-risk patch-level fixes: `store`'s `ajv` (ReDoS via `$data` option), `rider`'s `lodash` (code injection via `_.template`, prototype pollution via `_.unset`/`_.omit`) and `@babel/core` (arbitrary file read via sourceMappingURL), `admin`'s `postcss` (arbitrary `.map` file disclosure, XSS). All same-major patch/minor bumps, verified via fresh installs and (for admin, which deploys via a separate npm lockfile) `npm ci` in an isolated copy before adopting.

**Deliberately left alone, documented rather than chased:** a "critical"-labeled `shell-quote` advisory buried 5 levels deep in admin's `firebase>...>react-devtools-core` chain — dev-only React DevTools tooling firebase pulls in for RN compatibility, never reachable in the Next.js server runtime; and `cypress`'s own vulnerable transitive tree (`@cypress/request`, `extract-zip`, `qs`, `uuid`) — an E2E-testing devDependency that never ships to production.

### 18. [Info] Deprecated/EOL packages replaced
**Where:** `apps/api`, and the GraphQL-subscriptions client in all 4 apps

Two unmaintained packages were fully retired as part of the broader dependency-hygiene pass:

- **`apollo-server-express` v3 → `@apollo/server` v5** (`apps/api`): v3 was EOL and no longer receiving security patches. Migration required swapping in `@as-integrations/express4` (v5 ships no bundled framework integrations), moving `context` from the constructor into `expressMiddleware`'s second argument, and replacing every `ApolloError` (removed) with `GraphQLError` + `extensions.code`. Verified via a local boot against the real dev DB, then again against the exact production `npm install --omit=dev` path, before deploying.
- **`subscriptions-transport-ws` → `graphql-ws`** (`apps/api` + `apps/app`/`apps/store`/`apps/rider`/`apps/admin`): the old library has been unmaintained since ~2021 and doesn't speak a protocol any modern WebSocket-hardening guidance assumes. Migrated using a dual-protocol transition (the two libraries use different, incompatible WebSocket subprotocols) so the server ran both side by side while each of the 4 client apps was migrated and verified independently — avoiding a hard cutover that would have broken any already-installed mobile build still running the old client. Once all 4 clients were confirmed migrated and deployed, the old server-side path and dependency were removed entirely (`apps/api/app.js`, `apps/api/package.json`). Each client migration and the final server-side removal were verified with real WebSocket handshakes against production (confirming the new protocol connects and the old one is now correctly rejected, not just silently ignored) before being called done.
...
This pass covered `apps/admin`, `apps/app`, `apps/rider`, `apps/store`, and touched `apps/api` where client-side findings led directly back to a backend resolver (which is most of the high-severity items — client security in these apps is largely a function of what the shared backend allows). It was a manual, targeted audit (grep for secrets/insecure patterns + reading of auth/token/authorization code paths), not a penetration test.

**Update 2026-09-04:** the "not yet covered" dependency-CVE gap noted below has since been closed — see #16, #17, #18. A full `npm audit`/pnpm-audit sweep now covers all 5 apps (`app`, `store`, `rider`, `admin`, `api`), including the `apollo-server-express` and `subscriptions-transport-ws` EOL-package replacements. Still not covered: `apps/web`, exhaustive resolver-by-resolver authorization audit (only `order`/`rider`/`restaurant` single-entity queries were sampled), push notification token handling, and the Cloudinary unsigned upload preset's upload restrictions.
...
- **2026-08-24** — Started checking #11 in Google Cloud Console (browser automation, project `foodapp-503412`): found one "Maps Platform API Key" with Application restrictions set to "None" (Google's own UI flags this) and scoped to 35 APIs. But revealing the key showed it matches **neither** of the two Maps keys actually committed in `apps/app/app.json` - those belong to a different, inaccessible project (likely the vendor's). While tracing that mismatch, found something bigger: #15, both `apps/app` and `apps/store` pointed their production/staging backend URL at `aws-server.enatega.com`, a domain the user confirmed isn't theirs. Fixed #15 first (both now point to `foodapp-api.alaaeallam.com`, matching what `apps/rider` already had) since it was flagged higher urgency than #11. Confirmed via curl that the vendor domain currently fails TLS hostname validation, so impact was "production/staging builds are completely broken," not a live data leak - and no real builds are currently affected since the user isn't using TestFlight/App Store right now. #11 itself still open, resuming next: restrict the unused "foodapp" project key, then create new properly-restricted keys to replace the vendor-owned ones in `app.json`.
- **2026-08-17/18** — Ran the dependency/CVE audit against `apps/app`: fixed axios (SSRF/credential-leak CVEs), `mongoose`, `jsonwebtoken`, `nodemailer`, `firebase-admin`; removed dead weight (`eslint-plugin-standard`, `metro-react-native-babel-preset`, and `react-native-vector-icons` after confirming zero import sites). Migrated `apps/api` from `apollo-server-express` v3 (EOL, unpatched) to `@apollo/server` v5 — see #18.
- **2026-08-18** (continuing into September) — Began the `subscriptions-transport-ws` → `graphql-ws` migration (#18): server-side dual-protocol support landed in `apps/api/app.js` first (both WebSocket subprotocols routed side by side), then each client migrated one at a time — `apps/app`, then `apps/store`, then `apps/rider`, then `apps/admin` — each verified independently via real WebSocket handshake tests against production before moving to the next. Incidentally fixed a real bug found along the way: `editRider` never actually persisted a changed rider password; also fixed a schema-mismatch bug where the admin panel's `CREATE_RIDER`/`EDIT_RIDER` mutations requested a nonexistent `password` field on the `Rider` return type, and a production `DASHBOARD_URL` CORS misconfiguration that was silently breaking the admin panel's login.
- **2026-09-04** — All 4 clients confirmed migrated; removed the old `subscriptions-transport-ws` server path, dual-protocol routing, and dependency from `apps/api` entirely (#18 fully closed). Verified live against production both before and after removal (new protocol handshake succeeds, old protocol now correctly rejected rather than silently accepted).
- **2026-09-04** — Ran the dependency-CVE sweep across `apps/store`, `apps/rider`, `apps/admin` (closing the gap this doc's Scope note had flagged) — see #17. Found and fixed low-risk patch-level CVEs in each app, plus removed a dead `@types/react-native-vector-icons` devDependency from `store`. This surfaced #16: admin's `next@14.2.35` had ~21 CVEs with no fix anywhere in the 14.x line.
- **2026-09-04** — Fixed #16: upgraded admin to Next.js 15.5.25 + React 19.2.8 + next-intl 4.14.2, fixed the resulting async-API breaking changes, worked around two npm lockfile bugs the upgrade surfaced (`postcss`, `@swc/core` overrides), and verified via build/boot/production-deploy checks plus a full authenticated browser walkthrough once login access was restored. Also added `apps/api/scripts/reset-owner-password.js` (Owner-account equivalent of the existing `reset-rider-password.js`) after the admin account hit the login rate-limiter mid-session — a legitimate account-recovery utility, not a security gap (it requires direct server/`.env` access, same as its rider counterpart).
